...
contract StableMath {
    ...
    */
    ...
    function calculate(
        ...
        return _calcTokenOutGivenExactBptIn(
            amp,
            balances,
            tokenIndex,
            bptAmountIn,
    ...
    function for_usdc() external pure returns (uint256) {
        uint256[] memory tmp = new uint256[](3);
        // reserves in the FIAT-DAI-USDC pool
        tmp[0] = 867388558955302931346319;
        tmp[1] = 595625011149410642660316;
        tmp[2] = 579776360482 * 1e12;
    return _calcTokenOutGivenExactBptIn(
        200000, // amp factor
        tmp, // balances array
        2, // index 2 is USDC 1 is DAI 0 is FIAT
        10000000000000000000000, // bpt amount // 10K here
        2030864363627135171533309, // bpt token total supply
        100000000000000 // swap fee percentage
    );
}

function for_dai() external pure returns (uint256) {
        uint256[] memory tmp = new uint256[](3);
        // reserves in the FIAT-DAI-USDC pool
        tmp[0] = 867388558955302931346319;
        tmp[1] = 595625011149410642660316;
    ...
    return _calcTokenOutGivenExactBptIn(
        ...
        2030864363627135171533309,
        100000000000000
    );
}

function for_fiat() external pure  returns (uint256) {
        uint256[] memory tmp = new uint256[](3);
        // reserves in the FIAT-DAI-USDC pool
        tmp[0] = 867388558955302931346319;
        tmp[1] = 595625011149410642660316;
